...
int main(int argc, char *argv[])
{
    ...
    { // select assigned_core using first fit decreasing algorithm
        if (cpu_util[0] + tasks[i].util <= 1.0)
        { // If it fits in cpu 0, set assigned_core to 0
            cpu_util[0] += tasks[i].util;
            tasks[i].assigned_core = 0;
        }
        else if (cpu_util[1] + tasks[i].util <= 1.0)
        { // Else if it fits in cpu 1, set assigned_core to 1
            cpu_util[1] += tasks[i].util;
            tasks[i].assigned_core = 1;
        }
        else
        { // Else if it doesn't fit in either throw error
    ...
    for (int i = 0; i < rows_read; i++)
    { // Print which core each task is assigned to in order of id
