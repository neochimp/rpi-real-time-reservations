...
static void rsv_update_priorities(void)
{
    ...
    {                 // I couldn't find much but this is the best I've got - Documentation: https://lwn.net/Articles/537570/
        ...
        for (i = 0; i < MAX_RSV; i++)
        {
            ...
            if (!rsv_table[i].used)
            { // Verify that the task is being used
                continue;
            }

            temp = rsv_table[i].task; // Store the current task
            if (!temp || !temp->rsv_active)
            { // Verify that the current task exists and is active
                continue;
            }
            if (temp->rsv_cpu_id != cpu)
            { // Verify that rsv_cpu_id matches cpu
                continue;
            }
...
static void rsv_table_add(struct task_struct *curr_task, u64 T_ns)
{
    int i;
    for (i = 0; i < MAX_RSV; i++)
    { // find the first unused spot and insert there
        if (!rsv_table[i].used)
        {
